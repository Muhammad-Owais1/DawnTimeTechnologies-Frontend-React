...
export default function Header() {
  ...
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth >= 640) { // Assuming 640px as the sm breakpoint
        setNavOpen(false);
      }
    };
  ...
  return (
   <>
      ...
      <nav className={` w-full flex items-center justify-around flex-col sm:flex-row  sm:order-2  border-b-[1px] border-gray-200 {${navOpen ? "gap-0" : "gap-5"}} bg-white  top-0 z-10 sm:z-0 ${navFixed ? "fixed top-0 left-0 right-0" : ""}`}>
        <div className="flex items-center justify-between w-full sm:w-auto px-4 py-5">
          <img className="w-36" src={Logo} alt="" />
          <button onClick={() => setNavOpen(navOpen ? false : true)} className="flex sm:hidden">x</button>
        </div>
        <div className={`flex justify-center sm:visible sm:h-auto sm:gap-20 gap-3 flex-col sm:flex-row sm:py-0  ${navOpen ? "h-[271px] visible" : "h-0 invisible"}  overflow-hidden `}>
      ...
      <div className=" bg-slate-950  h-auto  p-4 sm:hidden flex items-center justify-around flex-col  gap-8  ">
        <img className="w-[200px]" src={BannerLogo} alt="" />
        <div className="flex fle-col items-center justify-center">
          <h1 className="font-black text-white sm:text-[40px] text-lg font-['Roboto'] italic ">
            WE ARE HERE.
